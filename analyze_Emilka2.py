...
# dfT = pd.read_hdf(BASE_DIR /"atm_neutrino_classB.h5")
# dfF = pd.read_hdf(files[6])

# for showers: classA classC classE classG
# dfS = pd.read_hdf(BASE_DIR /"atm_neutrino_classA.h5")
# dfC = pd.read_hdf(files[3])
# dfE = pd.read_hdf(files[5])
# dfG = pd.read_hdf(files[7])

df = pd.read_csv(BASE_DIR /"dataset_shower_balanced.csv")
dfS = df.sample(frac=1)
dfs = dfS.head(10000)
# dff = pd.read_csv(BASE_DIR /"dataset_track_balanced.csv")
dff = pd.read_hdf(BASE_DIR /"atm_muon.h5")
class1 = [0]*len(dff)
...
print(best_model)
joblib.dump(best_model, BASE_DIR/"best2_rf_model.joblib")
